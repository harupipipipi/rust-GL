...
impl Widget for Container {
    ...
    fn layout(
        ...
        let mut cx = x + f32_to_i32(self.style.padding.left);
        let mut cy = y + f32_to_i32(self.style.padding.top);
        ...
        for (i, child) in self.children.iter_mut().enumerate() {
            ...
            let cn = child.layout(cc, cx, cy, fonts);
            ...
            match self.style.direction {
                LayoutDirection::Vertical => {
                    cy += ch.round() as i32 + g.round() as i32;
                    used_main += ch + g;
                    max_cross = max_cross.max(cw);
                }
                LayoutDirection::Horizontal => {
                    cx += cw.round() as i32 + g.round() as i32;
                    used_main += cw + g;
